...
import { getUserDashboardInfo, getUserInfo } from "../../../lib/helper";
...
const UserDashboard = () => {
  ...
  const { data: userData } = useQuery(["user"], () => {
    if (userInfo._id) {
      return getUserInfo(userInfo._id);
    }
  });
        ...
        <div className="my-8 md:my-16 mx-2 ">
          ...
          <section className="p-6 my-6 mx-6 bg-gray-100  rounded-md text-gray-800">
            <div className="container grid grid-cols-1 gap-6 mx-auto sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="flex p-4 space-x-4 rounded-lg md:space-x-4 bg-white shadow-md text-gray-800">
                <div className="flex justify-center p-2 align-middle rounded-lg sm:p-4 bg-emerald-200">
                  <IoMdWallet size={35} />
                  {/* <Icon icon="solar:wallet-bold" className="text-emerald-500 text-[35px]" /> */}
                </div>
                <div className="flex flex-col justify-center align-middle">
                  <p className="text-3xl font-semibold leading-none">
                    {userData?.data?.wallet ? userData?.data?.wallet : 0} tk
                  </p>
                  <p className="capitalize">Cashback</p>
                </div>
              </div>
              <div className="flex p-4 space-x-2 rounded-lg md:space-x-4 bg-white shadow-md text-gray-800">
                <div className="flex justify-center p-2 align-middle rounded-lg sm:p-4 bg-emerald-200">
                  <SlWallet size={35} className="text-emerald-500" />
                </div>
                <div className="flex flex-col justify-center align-middle">
                  <p className="text-3xl font-semibold leading-none">
                    {userData?.data?.padding_wallet
                      ? userData?.data?.padding_wallet
                      : 0}{" "}
                    tk
                  </p>
                  <p className="capitalize">Pending Cashback</p>
                </div>
              </div>
            </div>
          </section>
