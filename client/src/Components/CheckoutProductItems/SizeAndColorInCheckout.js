...
const SizeAndColorInCheckout = ({ product, setIsOpen }) => {
  ...
  const handleSaveSizeInLocal = (inputSize) => {
  ...
  console.log("565855858",inputSize)
  ...
  return (
    <div>
          ...
          {product?.variant?.map((item, index) => (
            <>
              <div
                ...
                key={index}
                className={`mx-1 border items-center rounded-md font-bold ${item.size === inputSize.size
                    ? "bg-[#000] text-white"
                    : "bg-[#fff] text-black"
      ...
      <button
        onClick={() => {
          handleSaveSizeInLocal(inputSize);
          setIsOpen(false);
        }}
